...
export namespace dcc::sema
{
    ...
    private:
        ...
        template <typename P> static std::string type_str(P const* ty)
        {
            auto const* t = reinterpret_cast<types::Type const*>(ty);
            if (!t)
                return "<unresolved>";
            ...
            switch (t->kind)
            {
                ...
                case types::TypeKind::Int:
                    return std::format("{}{}", static_cast<types::IntType const*>(t)->is_signed ? 'i' : 'u',
                                       unsigned(static_cast<types::IntType const*>(t)->bits));
                case types::TypeKind::Float:
                    return std::format("f{}", unsigned(static_cast<types::FloatType const*>(t)->bits));
                case types::TypeKind::Char:
                    return "char";
                case types::TypeKind::NullT:
                    return "null_t";
                case types::TypeKind::Pointer: {
                    auto const* p = static_cast<types::PointerType const*>(t);
                    return std::format("ptr({}{})", p->pointee_quals == types::Qual::Const ? "const " : "", type_str(p->pointee));
                }
                case types::TypeKind::Array: {
                    auto const* a = static_cast<types::ArrayType const*>(t);
                    return std::format("[{}; {}]", type_str(a->element), a->count);
                }
                case types::TypeKind::Slice: {
                    auto const* s = static_cast<types::SliceType const*>(t);
                    return std::format("slice({}{})", s->element_quals == types::Qual::Const ? "const " : "", type_str(s->element));
                }
                ...
                case types::TypeKind::FuncPtr: {
                    ...
                    for (std::size_t i = 0; i < f->params.size(); ++i)
                    {
                        ...
                        params += type_str(f->params[i]);
                    }
                    return std::format("fn({}) -> {}", params, type_str(f->return_type));
                }
                case types::TypeKind::Struct:
                    return std::format("struct {}", decl_name(static_cast<types::StructType const*>(t)->decl));
                case types::TypeKind::Union:
                    return std::format("union {}", decl_name(static_cast<types::UnionType const*>(t)->decl));
                case types::TypeKind::Enum:
                    return std::format("enum {}", decl_name(static_cast<types::EnumType const*>(t)->decl));
                case types::TypeKind::Range:
                    return std::format("range({})", type_str(static_cast<types::RangeType const*>(t)->element));
                case types::TypeKind::RangeInclusive:
                    return std::format("range_inclusive({})", type_str(static_cast<types::RangeInclusiveType const*>(t)->element));
                case types::TypeKind::TemplateParam:
                    return std::format("{}#{}", static_cast<types::TemplateParamType const*>(t)->name, static_cast<types::TemplateParamType const*>(t)->index);
        ...
        switch (ty->kind)
        {
            ...
            case types::TypeKind::Pointer: {
                ...
                if (types::has_qual(p->pointee_quals, types::Qual::Restrict))
                    quals += "restrict ";
                return std::format("ptr({}{})", quals, format_type_str(p->pointee));
            }
            case types::TypeKind::Array: {
                auto const* a = static_cast<types::ArrayType const*>(ty);
                return std::format("[{}; {}]", format_type_str(a->element), a->count);
            }
            case types::TypeKind::Slice: {
                ...
                if (types::has_qual(s->element_quals, types::Qual::Restrict))
                    quals += "restrict ";
                return std::format("slice({}{})", quals, format_type_str(s->element));
            }
            case types::TypeKind::Fam:
            ...
            case types::TypeKind::FuncPtr: {
                ...
                for (std::size_t i = 0; i < f->params.size(); ++i)
                {
                    if (i)
                        params += ", ";
                    params += format_type_str(f->params[i]);
                }
                return std::format("fn({}) -> {}", params, format_type_str(f->return_type));
            }
            case types::TypeKind::Struct:
            case types::TypeKind::Union:
            case types::TypeKind::Enum: {
                ...
                std::string_view kind_name = (ty->kind == types::TypeKind::Struct) ? "struct " : (ty->kind == types::TypeKind::Union) ? "union " : "enum ";
                ...
                if (dd)
                {
                    ...
                    {
                        case ast::DeclKind::Struct:
                            name = static_cast<ast::StructDecl const*>(dd)->name;
                            break;
                        case ast::DeclKind::Union:
                            name = static_cast<ast::UnionDecl const*>(dd)->name;
                            break;
                        case ast::DeclKind::Enum:
                            name = static_cast<ast::EnumDecl const*>(dd)->name;
                            break;
                        default:
                            break;
                    }
                ...
                if (!ut->template_args.empty())
                {
                    ...
                    for (std::size_t i = 0; i < ut->template_args.size(); ++i)
                    {
                        if (i)
                            args += ", ";
                        args += format_type_str(ut->template_args[i]);
                    }
                    return std::format("{}{}<{}>", kind_name, name, args);
                }
                return std::format("{}{}", kind_name, name);
            }
            case types::TypeKind::Range:
                return std::format("range({})", format_type_str(static_cast<types::RangeType const*>(ty)->element));
            case types::TypeKind::RangeInclusive:
                return std::format("range_inclusive({})", format_type_str(static_cast<types::RangeInclusiveType const*>(ty)->element));
            case types::TypeKind::TemplateParam:
    ...
    private:
        ...
        {
            auto resolve_path = [&](ast::Path const& path) -> ast::UsingDecl const* {
                auto const* sym =
                    path.is_simple() ? scope.lookup_type(path.simple_name()) : (mod.own_scope ? resolve_type_path(*mod.own_scope, path) : nullptr);
                auto const* use = sym ? ast::node_cast<ast::UsingDecl>(sym->decl) : nullptr;
                return use && use->using_kind == ast::UsingKind::Concept ? use : nullptr;
            };

            if (auto const* id = ast::node_cast<ast::IdentExpr>(&callee))
            {
                if (auto const* sym = scope.lookup_type(id->name))
                    if (auto const* use = ast::node_cast<ast::UsingDecl>(sym->decl); use && use->using_kind == ast::UsingKind::Concept)
                        return use;

                if (mod.own_scope)
                    if (auto const* sym = mod.own_scope->lookup_type(id->name))
                        if (auto const* use = ast::node_cast<ast::UsingDecl>(sym->decl); use && use->using_kind == ast::UsingKind::Concept)
                            return use;
                                    ...
                                    types::TypePtr expected_type = nullptr, bool nttps_already_resolved = false, std::string* rejection_reason = nullptr)
        {
            ...
            if (!check_template_constraint(mod, scope, *func, b))
            {
                if (had_constraint_failure)
                    *had_constraint_failure = true;

                if (rejection_reason)
                    *rejection_reason = "constraint not satisfied";
                                                                          ...
                                                                          types::TypePtr expected_type = nullptr, std::string* rejection_reason = nullptr)
        {
            ...
            if (!check_template_constraint(mod, scope, f, b))
            {
                if (had_constraint_failure)
                    *had_constraint_failure = true;
        ...
        [[nodiscard]] PatternValidation validate_pattern(ModuleInfo& mod, ast::Pattern& p, types::TypePtr matched_type, Scope& scope, ConstEnv const* const_env)
        {
            ...
            switch (p.kind)
            {
                ...
                case ast::PatternKind::EnumDestructure: {
                    ...
                    auto const* sym = resolve_value_path(*mod.own_scope, e.variant_path);
        ...
        std::span<Symbol const> lookup_candidates(ModuleInfo const& mod, Scope const& local_scope, std::string_view name)
        {
            ...
            if (!mod.own_scope)
                return {};
            return mod.own_scope->lookup_values(name);
        }
        ...
        detail::ExprResult analyze_path_expr(ModuleInfo& mod, Scope& scope, ast::PathExpr& p, types::TypePtr expected_type, ConstEnv const* const_env)
        {
            ...
            {
                auto const* enum_sym = resolve_type_path(*mod.own_scope, p.path);
            ...
            auto const* sym = resolve_value_path(*mod.own_scope, p.path);
            if (!sym)
            {
                out.type = m_types.m_errort();
                                                 ...
                                                 std::uint32_t& next_off, types::TypePtr expected_type, ConstEnv const* const_env)
        {
            ...
            if (auto* path = ast::node_cast<ast::PathExpr>(t.callee))
            {
                ...
                if (mod.own_scope)
                {
                    auto const* sym = resolve_value_path(*mod.own_scope, path->path);
