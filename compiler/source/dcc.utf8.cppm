export module dcc.utf8;

import std;

export namespace dcc::utf8
{
    ...
    [[nodiscard]] constexpr int codepoint_width(Codepoint cp) noexcept
    {
        if (cp == 0)
            return 0;
    ...
    [[nodiscard]] constexpr std::expected<DecodeResult, Error> decode_one(std::span<const char8_t> bytes) noexcept
    {
        if (bytes.empty())
        ...
        {
            auto const b = static_cast<std::uint8_t>(bytes[i]);
            if ((b & 0xc0) != 0x80)
                return std::unexpected{Error::InvalidByte};
        ...
        return DecodeResult{static_cast<Codepoint>(cp), static_cast<int>(len)};
    }

    [[nodiscard]] constexpr std::expected<DecodeResult, Error> decode_one(std::string_view sv) noexcept
    {
    ...
    public:
        ...
        public:
            ...
            [[nodiscard]] value_type operator*() const noexcept { return decode_one(m_sv.substr(m_pos)); }
            ...
            CodepointIterator& operator++() noexcept
            {
                ...
                m_pos += r ? static_cast<std::size_t>(r->bytes_consumed) : 1;
                return *this;
            }
            ...
            [[nodiscard]] constexpr bool operator==(CodepointIterator const&) const noexcept = default;
    ...
    [[nodiscard]] constexpr std::expected<int, Error> string_width(std::string_view sv) noexcept
    {
        int width = 0;
        std::size_t pos = 0;
    ...
    [[nodiscard]] constexpr std::expected<WidthInfo, Error> string_width_info(std::string_view sv) noexcept
    {
        WidthInfo info{};
    ...
    [[nodiscard]] constexpr std::string_view truncate_to_width(std::string_view sv, int max_columns) noexcept
    {
        int cols = 0;
        std::size_t pos = 0;
